...
/**
 * @summary a intelligence logger, with @see WatchService that writes logs to a redis stream
 */
export class IntelligenceLogService {
    private key = '/logs/intelligence';
    private isStarted = false;
    watcher: WatchService;
    constructor(name: string, private redis: RedisService, private redisStream: RedisService,
        encryptKey: string = '', uniqueName = 'intelligenceLog', logReadWaitMS = 1000) {
        this.key = `/logs/${name}`
