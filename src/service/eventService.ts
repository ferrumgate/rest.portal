    ...
    constructor(private configService: ConfigService, private redisService: RedisService) {
        ...
        this.configService.events.on('changed', async (data: ConfigEvent) => {
            try {
                ...
                //await this.redisService.publish(`/config/changed`, Buffer.from(JSON.stringify(simpleEvent)).toString('base64url'));
                await this.redisService.publish(`/config/changed`, Util.jencode(simpleEvent).toString('base64url'));
            } catch (err) {
                logger.error(err);
            }
